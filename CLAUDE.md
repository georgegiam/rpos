...
- [x] **A6 Churn:** lookup success and availability vs session length. → #37;
      `experiments/run_a6.sh` (collect: N=32 s=3 @ 10 qps, sessions {30,60,120,300,∞} ×3, 120 s each,
      churn via the new `experiments/a6_churn_injector.py`) + `experiments/a6_churn.py` (client↔node
      join → raw-DHT headline + `sim/churn_sim.py` cross-check + plot) → `results/A6_churn.csv` +
      `fig_A6_churn.png` + `A6_NOTES.md`. **Enabler (no frozen edit):** a host-side churn injector
      that `docker kill`/`docker start`s ring containers on a seeded per-node alternating-renewal
      schedule (node 0 = seed never churned); measured queries enter at node 0 (`query_gen
      --ring-nodes 1`) to isolate data/route availability. **Result:** raw-DHT lookup success falls
      **76.3→70.7%** as sessions shorten ∞→30 s (churn effect = drop below the ∞ cold-start anchor,
      which matches A1's N=32 split); **end-to-end success stays ≥96%** (DNS-hierarchy fallback
      recovers churn losses); p95 ≈2× under heavy 30 s churn (929→1693 ms, tail-only — p50 cache-flat
      1.6–2.9 ms); mean-live-pop tracks the injector (27.8→32.0). Sim cross-check 96.9→100%
      (data-availability model, 1 s repair, no cold-start → more optimistic, directional not fitted).
      `node/chord.py`/`node/storage.py`/`rpos/rpos.py`/`sim/churn_sim.py` byte-identical.
